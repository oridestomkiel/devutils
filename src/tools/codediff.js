const codediff = {
  title: "Diff de Código (GitHub Style)",
  description:
    "Compare dois blocos de código e visualize as diferenças com estilo GitHub lado a lado.",
  tags: ["diff visual", "git", "comparar código", "diff2html"],
  ...
  render: () => `
      ...
      <div id="diffHtmlOutput" class="mt-4 text-sm overflow-auto"></div>
    `,
  init: () => {
    ...
    document.getElementById("diffCompareBtn").addEventListener("click", () => {
      const original = document.getElementById("diffOriginal").value.trim();
      const modified = document.getElementById("diffModified").value.trim();
      ...
      loadDiffLibs(() => {
        ...
        document.getElementById("diffHtmlOutput").innerHTML = html;
      });
    });
  },
};
